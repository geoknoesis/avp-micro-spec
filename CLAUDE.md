...
- **`status.py`**: Bitstring Status List v1.0 codec — deterministic `encodedList` build/read (set revoked bits, GZIP `mtime=0`, base64url, multibase `u`). Used by the Authority bundle for the `BitstringStatusListCredential` status vectors.
